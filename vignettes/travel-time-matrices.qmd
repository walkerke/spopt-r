...
# We used osmium to clip the US South extract to Tarrant County:
...
Each demand point is colored by its assigned facility. Notice how the service areas follow the road network structure rather than forming simple circular regions.
...
-   **Cache your matrices**: Travel-time computation is expensive. Save matrices with `saveRDS()` for reuse.

-   **Consider asymmetry**: Driving times can differ by direction (one-way streets, turn restrictions). For most facility location problems, you may want to average both directions.
